...
function useThrottle<T>(value: T, delay = 500){
    ...
    useEffect(() => {
        ...
        if (Date.now() >= lastExecuted.current + delay) {
            ...
            // 최신 value 를 throttledValue 에 저장
            setThrottledValue(value);
        } else {
            // 충분한 시간이 지나지 않은 경우, delay 시간 후에 업데이트, 최신 value 로
            ...
            // 기존 타이머를 clearTimeout 을 통해 취소하여 중복 업데이트 방지
