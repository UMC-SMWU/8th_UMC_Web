...
axiosInstance.interceptors.request.use((config) => {
    ...
    const accessToken = getItem(); // localStorage에서 accessToken 가져온다.
...
//응답 인터셉터 : 401 에러 발생 => refreshToken을 통한 토큰 갱신을 처리합니다.
axiosInstance.interceptors.response.use(
    (response) => response,
    async (error) => {
        const originalRequest : CustomInternalAxiosRequestConfig = error.config;

        // 401에러면서, 아직 재시도하지 않은 요청 경우 처리
        if (
            error.response && 
            ...
            if (!refreshPromise) {
                ...
                refreshPromise = null; // 요청이 끝나면 프로미스를 초기화
            });
        }
